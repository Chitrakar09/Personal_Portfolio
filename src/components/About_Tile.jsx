...
import { css, tailwind, html, react, javascript, git, figma } from '@/assets/icon_index'
...
function About_Tile() {
    ...
    return (
        <div id="container" className='bg-[linear-gradient(180deg,#0A2540_0%,#000000_100%)] min-w-full min-h-full p-5 rounded-3xl flex flex-col'>
            <div id="profile" className='w-full flex flex-col justify-center items-center bg-[#2c2c2c]/30 px-2.5 py-8 rounded-2xl gap-1'>
                ...
                <div id="tech_stack" className='w-full flex justify-center gap-3 items-center'>
                    {icons.map((item)=>
                <img src={item.icon} alt={`${item.icon}`} key={`${item.icon}`} className='w-3 ms:w-5 h-auto'/>    
                )}
                </div>
            </div>
        </div>
